...
\usepackage{pdfpages} % To include bibliography.pdf
...
We can also rearrange \Cref{eq:vrotcummass} to get the internal mass \(M\) of the galaxy up to any radial distance \(r\):
\begin{equation}\label{eq:internalmass}
    M = \frac{r\vrot^2}{G}.
\end{equation}
...
Uncertainty is inevitable when dealing with astronomical data, since even the best of measuring instruments cannot guarantee full certainty.
The SPARC database from which the \(\vobs\) data was taken provided the \(r\)-values up to two decimal places, but did not provide an uncertainty.
Therefore I was obliged to take an uncertainty of \(\pm 0.01\), which is present in all of the table.
The uncertainty in \(\vobs\) was provided, however, as was the uncertainty in \(\theta\), the latter being from the NASA/IPAC Extragalactic Database.
These uncertainties originate from the limitations of the technology and methods of measurement used.
The values are as presented in the corresponding tables.
The uncertainties in the dependent variables, namely \(\vrot\) and \(M\), were computed according to
\begin{equation}
    \Delta \vrot = \frac 1 2 \left(\frac{\vobs + \Delta\vobs}{\sin(\theta - \Delta\theta)} - \frac{\vobs - \Delta\vobs}{\sin(\theta + \Delta\theta)} \right), \
    \qquad \
    \Delta M = \left(2\frac{\Delta\vrot}{\vrot} + \frac{\Delta r}{r}\right)M.
\end{equation}
These are derived from \Cref{eq:vrotfromvobs,eq:internalmass}, respectively.

\includepdf[pages={1}]{bibliography.pdf}
